            ...
            if request.method == "GET":
                ...
                if form.is_valid():
                    ...
                    rejectCounter = list(InboundData.objects.filter(
                        inbound=request.session['id_inbound'], userGroup=request.session['usergroup'], deleted=0).values('rejectCounter', 'item'))
...
def supplierReturnConfirm(request):
    if 'is_login' not in request.session or request.session['limit'] <= datetime.datetime.today().strftime('%Y-%m-%d'):
        ...
        if request.session['role'] == "OPR":
            ...
            supplier_returndata = list(SupplierReturnData.objects.filter(
                supplierReturn=request.session['supplierReturn'], userGroup=request.session['usergroup'], deleted=0).values_list('id', 'item'))
            inbounddata_id_list = list(InboundData.objects.filter(
                inbound=request.session['inbound_id'], userGroup=request.session['usergroup'], deleted=0).values_list('id', 'quantity', 'rejectCounter', 'item'))
            rejectlist = list(InboundData.objects.filter(inbound=request.session['inbound_id']).exclude(
            ...
            for i in inbounddata_id_list:
                while j < len(supplier_returndata):
                    if supplier_returndata[j][1] == inbounddata_id_list[3]:
                        data.append(ItemData(id='ITD' + str(get_next_value('itemdata_seq')), inbound=InboundData.objects.get(
                            pk=i[0]), userGroup=UserGroup.objects.get(pk=request.session['usergroup'])))
                        print(data)
